...
from .pages import intake, workroom

ROUTES = {
    "intake": "Products",
    "workroom": "Analysis workroom",
...
def _sidebar(service: StudioService) -> str:
    with st.sidebar:
        ...
        route = st.radio(
            ...
            format_func=lambda key: ROUTES[key],
            index=list(ROUTES).index(st.session_state.get("route", "intake")),
        ...
        active = len([j for j in _active_jobs()])
        if active:
            st.info(f"{active} analysis running", icon=":material/sync:")
